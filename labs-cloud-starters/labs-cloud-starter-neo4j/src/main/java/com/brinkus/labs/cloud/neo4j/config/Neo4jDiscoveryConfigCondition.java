...
public class Neo4jDiscoveryConfigCondition implements Condition {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(Neo4jDiscoveryConfigCondition.class);

    @Override
    public boolean matches(final ConditionContext context, final AnnotatedTypeMetadata metadata) {
        String property = context.getEnvironment().getProperty("labs.neo4j.enabled");
        boolean isEnabled = Boolean.parseBoolean(property);
        LOGGER.info("Neo4j discovery configuration is enabled: {}", isEnabled);
        return isEnabled;
    }
}
